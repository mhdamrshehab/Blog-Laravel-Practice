...
<body style="font-family: Open Sans, sans-serif">
            ...
            <div class="mt-8 md:mt-0 flex items-center justify-center">
                @auth
                    ...
                    <form action="/logout" method="POST">
                        @csrf
                        <button type="submit" class="text-xs font-bold uppercase text-blue-500 ml-6">Log Out</button>
                    </form>
                @else
    ...
    @if (session()->has('success'))
        <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 4000)" x-show="show"
            class="fixed bg-blue-500 bottom-3 right-3 text-white py-2 px-4 rounded-xl text-sm ">
            <p>
                {{ session('success') }}
            </p>
        </div>
